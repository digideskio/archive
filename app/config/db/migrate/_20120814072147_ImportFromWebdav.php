  	...
  	foreach (new DirectoryIterator($webdav) as $collection) {
  		...
  		if(is_dir($collection_path)) { // Only look in directories
			...
			if($csvs) {
				...
				exec("find -maxdepth 1 -type f -print0 | xargs -0 file -i | grep -i -E '($grep_mimes)'", $finds);
				...
				foreach($finds as $f) {
					...
					// The following two lines generate the thumbnails on the fly
					...
					//The following lines assume the thumbnails already exist
					$thumb_name = $file_title . '.jpeg';
					$the_thumb = $collection_dir . DIRECTORY_SEPARATOR . 'thumb' . DIRECTORY_SEPARATOR . $thumb_name;
					$the_small = $collection_dir . DIRECTORY_SEPARATOR . 'small' . DIRECTORY_SEPARATOR . $thumb_name;
					copy($the_thumb, $thumb);
					copy($the_small, $small);
					...
					if($artwork) {
						...
						$collection_work = $this->select_one("SELECT * FROM collections_works WHERE collection_id = '$collection_id' AND work_id = '$work_id'");
