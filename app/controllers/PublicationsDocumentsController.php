...
class PublicationsDocumentsController extends \lithium\action\Controller {
	...
	public function add() {
		...
		if ($this->request->data) {
			...
			$publicationsDocument = PublicationsDocuments::create();
			...
			if($document) {
				...
				$publicationsDocument->save($this->request->data);
	...
	public function delete() {
		...
		if (!$this->request->is('post') && !$this->request->is('delete')) {
			$msg = "PublicationsDocuments::delete can only be called with http:post or http:delete.";
			throw new DispatchException($msg);
		}
		
		//Dont run the query if no id is provided
		if(isset($this->request->params['id'])) {
			PublicationsDocuments::first(array(
